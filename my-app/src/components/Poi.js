...
function Poi() {
    ...
    const my_data =  {
        ...
        return (
            ...
            (<div id='rec-list'>
                {poi.map((place, index) => (
                    <div key={index} className="rec" onClick={(event)=>{makeSelection({...select,outbound:{...place,local_id:index}});document.getElementsByClassName('rec')[index].style.backgroundColor="green";}}>
                        {place['name']} 
                        <br></br>
                        Hours: {place['hours']}
                        <br></br>
